    ...
    <Dialog open={open} onOpenChange={handleOpenChange}>
      ...
      <DialogContent className="h-[94vh] w-[96vw] max-w-[96vw] gap-0 overflow-hidden p-0">
        ...
        <form onSubmit={handleSubmit} className="flex min-h-0 flex-1 flex-col overflow-hidden">
          ...
          <div className="min-h-0 flex-1 overflow-auto p-4">{loadingReport ? <div className="flex h-40 items-center justify-center text-muted-foreground"><Loader2 className="mr-2 h-5 w-5 animate-spin" />Loading report...</div> : <PathologyReportEditor key={`${reportId || "new"}-${open}`} branchId={activeBranchId} labId={activeLabId} initialTestNames={initialTestNames} initialRows={initialRows} onChange={setPayload} />}</div>
